defmodule NixosTest.StartCommandTest do
  ...
  describe "name/1" do
    test "extracts machine name from nix start script path" do
      assert StartCommand.name("/nix/store/abc-nixos-test-driver/bin/run-server-vm") == "server"
    end
  ...
  describe "to_machine_config/1" do
    test "converts build result to a machine config map" do
      ...
      assert config.backend == NixosTest.Machine.Backend.QEMU
